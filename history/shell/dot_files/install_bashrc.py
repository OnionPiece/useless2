...
def main():
    #if len(sys.argv) != 2 or sys.argv[1] not in ('-a', '-u'):
    #    usage()
    #    sys.exit(1)
    ...
    #action_msg = {'-a': 'appending', '-u': 'updating'}[sys.argv[1]]
    action_msg = 'updating'
    if not os.path.exists(bashrc_path):
        print "no %s found, creating a new one..." % bashrc_path
        open(bashrc_path, 'w+').close()
    else:
        print "before %s %s, copy existing it as %s.backup" % (
            action_msg, bashrc_path, bashrc_path)
    ...
    print '%s %s complete, you should run `source %s`' % (
        bashrc_path, action_msg, bashrc_path)
